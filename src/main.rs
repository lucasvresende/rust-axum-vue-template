...
async fn login(State(s): State<AppState>, Json(b): Json<Login>) -> Result<Json<Token>> {
    let row = sqlx::query!("SELECT id, email, full_name, is_active, is_superuser, hashed_password FROM users WHERE email = $1", b.email.to_lowercase()).fetch_optional(&s.db).await?.ok_or(ApiError::Unauthorized)?;
...
async fn register(
    ...
    let u = sqlx::query_as!(User, "INSERT INTO users (id, email, hashed_password, full_name) VALUES ($1, $2, $3, $4) RETURNING id, email, full_name, is_active, is_superuser", Uuid::new_v4(), b.email.to_lowercase(), hash(&b.password)?, b.full_name.unwrap_or_default()).fetch_one(&s.db).await?;
...
async fn update_me(
    ...
    Ok(Json(sqlx::query_as!(User, "UPDATE users SET full_name = $1 WHERE id = $2 RETURNING id, email, full_name, is_active, is_superuser", b.full_name, u.id).fetch_one(&s.db).await?))
}
async fn users(State(s): State<AppState>, h: axum::http::HeaderMap) -> Result<Json<Vec<User>>> {
    if !auth(&h, &s).await?.is_superuser {
        return Err(ApiError::Forbidden);
    }
    Ok(Json(sqlx::query_as!(User, "SELECT id, email, full_name, is_active, is_superuser FROM users ORDER BY created_at DESC").fetch_all(&s.db).await?))
}
async fn items(State(s): State<AppState>, h: axum::http::HeaderMap) -> Result<Json<Vec<Item>>> {
    let u = auth(&h, &s).await?;
    Ok(Json(sqlx::query_as!(Item, "SELECT id, title, description, owner_id FROM items WHERE owner_id = $1 ORDER BY created_at DESC", u.id).fetch_all(&s.db).await?))
}
async fn create_item(
    ...
    let u = auth(&h, &s).await?;
    let i = sqlx::query_as!(Item, "INSERT INTO items (id, title, description, owner_id) VALUES ($1, $2, $3, $4) RETURNING id, title, description, owner_id", Uuid::new_v4(), b.title, b.description.unwrap_or_default(), u.id).fetch_one(&s.db).await?;
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    if sqlx::query!("SELECT id FROM users WHERE email = $1", email)
        ...
        .is_none()
    {
        sqlx::query!("INSERT INTO users (id, email, hashed_password, full_name, is_superuser) VALUES ($1, $2, $3, $4, TRUE)", Uuid::new_v4(), env::var("FIRST_SUPERUSER_EMAIL").unwrap_or_else(|_| "admin@example.com".into()), hash(&env::var("FIRST_SUPERUSER_PASSWORD").unwrap_or_else(|_| "changethis".into()))?, "Administrator").execute(&db).await?;
    }
