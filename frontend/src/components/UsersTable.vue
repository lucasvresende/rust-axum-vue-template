...
const indexedRows = computed(() => props.users.map((row, index) => ({ ...row, rowIndex: index + 1 })));
const first = ref(0);
</script>
...
<template>
    ...
    >
        ...
        <template #content>
            <DataTable v-model:first="first" :value="indexedRows" paginator :rows="10" :rowsPerPageOptions="[5, 10, 20]" dataKey="id" scrollable>
