...
test("edit item and require confirmation before deleting", async ({ page }) => {
  ...
  let items = [{ id: "item", owner_id: "member", title: "Bolts", description: "Steel", quantity: 12, created_at: "2026-09-20T12:00:00Z" }];
  let deletes = 0;
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (route.request().method() === "PUT") {
      ...
      expect(route.request().postDataJSON()).toEqual({ title: "Nuts", description: "Brass", quantity: 5 });
      items = [{ ...items[0], ...route.request().postDataJSON() }];
      return route.fulfill({ json: items[0] });
    }
    if (route.request().method() === "DELETE") {
      deletes++;
      if (deletes === 1) return route.fulfill({ status: 500, json: { detail: "Try again" } });
      items = [];
  ...
  await expect(page.getByRole("row").filter({ hasText: "Nuts" }).locator("time")).toHaveAttribute("datetime", "2026-09-20T12:00:00Z");
  const openDelete = async () => {
    await page.getByRole("button", { name: "Actions for Nuts" }).click();
    await page.getByRole("menuitem", { name: "Delete", exact: true }).click();
  };
  await openDelete();
  const confirmation = page.getByRole("dialog", { name: "Delete item", exact: true });
  ...
  await confirmation.getByRole("button", { name: "Delete item", exact: true }).click();
  await expect(confirmation).not.toBeVisible();
  await expect(page.getByText("No items yet. Create your first one.")).toBeVisible();
